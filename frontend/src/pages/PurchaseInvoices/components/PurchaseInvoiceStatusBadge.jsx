...
const PurchaseInvoiceStatusBadge = ({ status, statusName }) => {
  console.log("status -> ", status);

  const getStatusColor = (status) => {
    switch (parseInt(status)) {
      case PURCHASE_INVOICE_STATUS.DRAFT:
        return "bg-gray-200 text-gray-700"; // Draft
      case PURCHASE_INVOICE_STATUS.PENDING:
        return "bg-yellow-100 text-yellow-700"; // Pending
      case PURCHASE_INVOICE_STATUS.APPROVED:
        return "bg-green-100 text-green-700"; // Approved
      case PURCHASE_INVOICE_STATUS.CANCELLED:
...
export default PurchaseInvoiceStatusBadge;
